...
function configurarEventos() {
    ...
    // Foto da OS
    const inputFotoOS = document.getElementById('fotoOS');
    if (inputFotoOS) {
        inputFotoOS.addEventListener('change', onFotoOSChange);
    }
...
// ============================================
// FOTOS DA OS - SELEÇÃO E RENDERIZAÇÃO
// ============================================
function onFotoOSChange(e) {
    ...
    if (!files.length) {
        if (state.fotosSelecionadas.length === 0) {
            lista.innerHTML = '<p class="empty-message">Nenhuma foto selecionada</p>';
        }
        return;
    }
    ...
    // Adiciona os arquivos válidos no estado
    files.forEach((file) => {
        if (file.size > tamanhoMax) {
            showToast(`"${file.name}" ultrapassa 5MB e foi ignorado`, 'warning');
            return;
        }
        state.fotosSelecionadas.push(file);
    });

    // Limpa o input pra permitir selecionar mais depois (inclusive as mesmas)
    e.target.value = '';
...
function renderizarFotos() {
    ...
    if (!state.fotosSelecionadas.length) {
        lista.innerHTML = '<p class="empty-message">Nenhuma foto selecionada</p>';
        return;
    }
    ...
    state.fotosSelecionadas.forEach((file, index) => {
        ...
        reader.onload = function (event) {
            ...
            card.innerHTML = `
                <div class="item-header">
                    <span class="item-nome">${file.name}</span>
                    <button type="button" class="btn-danger" onclick="removerFoto(${index})">
                        🗑️ Remover
                    </button>
                </div>
                <div class="item-body">
                    <div class="item-field">
                        ...
                        <img src="${event.target.result}" 
                             style="max-width: 120px; border-radius: 8px; border: 1px solid #ddd; padding: 2px;">
                    </div>
                    <div class="item-field">
                        <label>Tamanho</label>
                        <div class="item-subtotal">${tamanhoKB} KB</div>
                    </div>
                </div>
            `;
...
function removerFotoSelecionada() {
    const inputFoto = document.getElementById('fotoOS');
    const lista = document.getElementById('listaFotos');

    if (inputFoto) inputFoto.value = '';
    if (lista) lista.innerHTML = '<p class="empty-message">Nenhuma foto selecionada</p>';

    showToast('Foto removida', 'info');
}


async function uploadFotoOS(osId) {
    if (!state.fotosSelecionadas.length) {
        // Nenhuma foto selecionada -> não faz nada
        return;
    }

    for (const file of state.fotosSelecionadas) {
        const formData = new FormData();
        formData.append('file', file); // campo "file" por causa do multer
        formData.append('os_id', osId);
        formData.append('descricao', 'Foto anexada na abertura da OS');
        ...
        try {
            ...
            if (!response.ok || !data.sucesso) {
                console.warn('Erro ao enviar foto:', data);
                showToast(data.erro || `Erro ao enviar a foto "${file.name}"`, 'warning');
            }
        } catch (error) {
            console.error('Erro ao fazer upload da foto:', error);
            showToast(`Erro ao enviar a foto "${file.name}"`, 'warning');
        }
    }
...
function mostrarModalVeiculo() {
    // Lê sempre o cliente diretamente do select
    const clienteId = document.getElementById('cliente').value;
    ...
    // Garante que o estado fique alinhado com o select
    state.clienteAtual = clienteId;
...
async function salvarVeiculo() {
 ...
 // Garante cliente antes de tudo
    let clienteId = state.clienteAtual || document.getElementById('cliente').value;

    if (!clienteId) {
        showToast('Selecione um cliente antes de cadastrar um veículo', 'warning');
        return;
    }
...
async function salvarOS(event) {
    ...
    try {
        ...
        if (data.sucesso) {
            ...
            // Pega o ID da OS retornada
            const osId = data.os?.id || data.id;

            // Se tiver OS id, tenta subir a foto (se houver)
            if (osId) {
                await uploadFotoOS(osId);
            }
...
function limparFormulario() {
    if (confirm('Deseja realmente limpar o formulário?')) {
        ...
        // Limpar foto e lista de fotos
        const inputFoto = document.getElementById('fotoOS');
        const listaFotos = document.getElementById('listaFotos');
        if (inputFoto) inputFoto.value = '';
        if (listaFotos) {
            listaFotos.innerHTML = '<p class="empty-message">Nenhuma foto selecionada</p>';
        }
