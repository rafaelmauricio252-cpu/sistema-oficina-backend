...
async function criarOS(req, res) {
  try {
    const {
      ...
      veiculo_id,
      mecanico_id,
      data_abertura, // Agora opcional, se não vier será new Date()
    ...
    // Definir data de abertura automática se não fornecida
    const dataAberturaFinal = data_abertura || new Date();

    const osInserida = await db.transaction(async (trx) => {
      // PASSO 0: Validações de regras de negócio

      // Regra 1: Validar data de agendamento (permite hoje e futuro, não permite passado)
      if (status === 'Agendamento') {
        if (!data_agendamento) {
          throw new Error('Data de agendamento é obrigatória para status Agendamento');
        }
        // Extrair apenas a parte da data (YYYY-MM-DD) para comparação
        const dataAgendamentoStr = data_agendamento.split('T')[0]; // "2025-12-01"
        const hojeStr = new Date().toISOString().split('T')[0]; // "2025-12-01"

        // Comparar strings de data diretamente (formato ISO permite comparação alfabética)
        if (dataAgendamentoStr < hojeStr) {
          throw new Error('Data de agendamento não pode ser anterior à data atual');
        }
      }

      // Regra 1.5: Validar forma de pagamento ao criar OS com status Pago
      if (status === 'Pago' && (!forma_pagamento || forma_pagamento.trim() === '')) {
        throw new Error('Forma de pagamento é obrigatória ao criar OS com status Pago');
      }

      // Regra 2: Validar se mecânico já tem OS "Em Andamento"
      ...
      // Regra 3: Validar datas (data_conclusao >= data_abertura)
      if (data_conclusao) {
      ...
      const [novaOS] = await trx('ordem_servico').insert({
        cliente_id, veiculo_id, mecanico_id,
        data_abertura: dataAberturaFinal,
        data_agendamento: status === 'Agendamento' ? data_agendamento : null,
    ...
    if (erro.message.includes('Estoque insuficiente') ||
      ...
      erro.message.includes('Data de agendamento') ||
...
async function atualizarOS(req, res) {
  try {
    ...
    const osAtualizada = await db.transaction(async (trx) => {
      ...
      // ============================================
      // PROTEÇÃO: Bloquear edição de OS Pago
      // ============================================
      if (osExistente.status === 'Pago') {
        throw new Error('Não é possível editar ordem de serviço com status "Pago"');
      }

      // Validações de regras de negócio
      if (status === 'Pago' && (!forma_pagamento || forma_pagamento.trim() === '')) {
        throw new Error('Forma de pagamento é obrigatória ao alterar status para Pago');
      }

      // Regra Automática: Ao marcar como Pago, registrar data de fechamento se não fornecida
      let dataFechamentoFinal = data_conclusao;
      if (status === 'Pago' && !data_conclusao && !osExistente.data_fechamento) {
        dataFechamentoFinal = new Date();
      }
      ...
      // Regra 3: Validar data de agendamento se status for Agendamento
      if (status === 'Agendamento' && data_agendamento) {
        // Extrair apenas a parte da data (YYYY-MM-DD) para comparação
        const dataAgendamentoStr = data_agendamento.split('T')[0];
      ...
      if (servicos !== undefined) {
        ...
        for (const servicoNovo of servicos) {
          ...
          // Aceitar tanto preco_servico quanto preco_unitario (compatibilidade frontend)
          const precoServico = servicoNovo.preco_servico || servicoNovo.preco_unitario;

          if (servicoExistenteNaOS) {
            await trx('os_servicos')
              .where({ os_id: id, servico_id: servicoNovo.servico_id })
              .update({ preco_servico: precoServico });
          } else {
      ...
      if (data_agendamento !== undefined) camposParaAtualizar.data_agendamento = data_agendamento;
      if (dataFechamentoFinal !== undefined) camposParaAtualizar.data_fechamento = dataFechamentoFinal;
    ...
    if (erro.message.includes('Ordem de Serviço não encontrada') ||
      ...
      erro.message.includes('Desconto não pode') ||
      erro.message.includes('Não é possível editar ordem de serviço') ||
      erro.message.includes('Data de agendamento')) {
      return res.status(400).json({ erro: erro.message });
    }
...
async function deletarOS(req, res) {
  try {
    ...
    const osDeletada = await db.transaction(async (trx) => {
      ...
      if (osExistente.status === 'Pago') {
        throw new Error('Não é possível excluir ordem de serviço com status "Pago"');
      }
